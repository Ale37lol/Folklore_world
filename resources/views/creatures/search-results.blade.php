...
@section('content')
    <div class="mb-8 pl-4 pr-4">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <form action="{{ route('creatures.search') }}" method="GET" class="w-full md:w-auto">
                <input type="text" name="search" value="{{ $searchQuery }}" placeholder="Search creatures..."
    ...
    @else
                            ...
                            @else
                                ...
                                @if (file_exists($fullPath))
                                    <img src="{{ asset($relativePath) }}" alt="{{ $creature->name }}"
                                        class="object-cover w-full h-full" style="object-fit: cover; object-position: top;">
                                @else
                                    <span class="text-gray-500 text-sm">Image not found for {{ $filename }}</span>
                                @endif
                            @endif
                        </div>
                        <div class="p-4">
        ...
        <div class="mt-8">
            {{ $creatures->appends(['search' => $searchQuery])->links() }}
        </div>
    @endif
@endsection
